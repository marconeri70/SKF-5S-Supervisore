// app.js — v2.3.16 (fix raggruppamento CH cross-browser)
(() => {
  console.log('SKF 5S Supervisor app.js v2.3.16');
  ...
  function parseRec(o){
    ...
    if (Array.isArray(o.notes)){
      r.notes = o.notes.map(n=>({ s:n.s||n.S||n.type||'', text:n.text||n.note||'', date:n.date||r.date }));
    }
    return r;
  }
  ...
  function renderDelays(){
    ...
    for (const r of data){
      const k = r.channel;
      if (!lastByCH.has(k)) lastByCH.set(k, []);
      lastByCH.get(k).push(r);
    }
    const rows = [];
    const now = Date.now();
    for (const [ch, arr] of lastByCH){
      ...
      if (days > 7) rows.push({ch, days, last});
    }
  ...
  function renderHome(){
    ...
    console.log('Loaded records (filtered):', data.length);
  ...
  function renderChecklist(){
    ...
    for (const [ch, arr] of Array.from(byCh.entries()).sort()){
      ...
      card.querySelector('.btn-toggle-one').onclick = (e) => {
        ...
        e.currentTarget.textContent = card.classList.contains('compact') ? 'Espandi' : 'Comprimi';
      };
    }
